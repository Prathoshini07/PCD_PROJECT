...
MultiplyEqualsNode::MultiplyEqualsNode(IdentifierNode *in, ExpressionNode *en)
    : AssignmentStatementNode(in, en) {}

void MultiplyEqualsNode::Interpret() {
    int val = this->IDNode->Evaluate() * this->expNode->Evaluate();
    this->IDNode->SetValue(val);
}

void MultiplyEqualsNode::Code(InstructionsClass &machineCode) {
    int index = this->IDNode->GetIndex();
    machineCode.PushVariable(index);
    this->expNode->CodeEvaluate(machineCode);
    machineCode.PopPopMulPush();
...
DivideEqualsNode::DivideEqualsNode(IdentifierNode *in, ExpressionNode *en)
    : AssignmentStatementNode(in, en) {}

void DivideEqualsNode::Interpret() {
    int divisor = this->expNode->Evaluate();
    if (divisor == 0) {
        throw std::runtime_error("Division by zero error.");
    }
    int val = this->IDNode->Evaluate() / divisor;
    this->IDNode->SetValue(val);
}

void DivideEqualsNode::Code(InstructionsClass &machineCode) {
    int index = this->IDNode->GetIndex();
    machineCode.PushVariable(index);
    this->expNode->CodeEvaluate(machineCode);
